...
#include "../../Compute/FileReader/FileReader.hpp"
...
GraphicsPipeline::GraphicsPipeline(Device &device, ImageViews &imageViews, DescriptorSetLayout &descriptorSetLayout)
{
    ...
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    viewport.width = (float) imageViews.getSwapChainExtent().width;
    viewport.height = (float) imageViews.getSwapChainExtent().height;
    viewport.minDepth = 0.0f;
    viewport.maxDepth = 1.0f;

    VkRect2D scissor{};
    scissor.offset = {0, 0};
    scissor.extent = imageViews.getSwapChainExtent();
...
//    rasterizer.frontFace = VK_FRONT_FACE_CLOCKWISE;
    ...
    if (vkCreateGraphicsPipelines(device.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &this->graphicsPipeline) != VK_SUCCESS) {
...
void GraphicsPipeline::createRenderPass(Device &device, ImageViews &imageViews)
{
    VkAttachmentDescription colorAttachment{};
    colorAttachment.format = imageViews.getSwapChainImageFormat();
    ...
    depthAttachment.format = DepthResources::findDepthFormat(device);
